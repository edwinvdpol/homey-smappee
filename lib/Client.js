...
class Client extends OAuth2Client {
  ...
  */
  ...
  async getInitialSensorConsumption(id, serviceLocationId) {
    const history = await this.getHistoricalSensorConsumption(id, serviceLocationId);
  ...
  async getHistoricalSensorConsumption(id, serviceLocationId) {
    const records = await this.getSensorConsumption(id, serviceLocationId, Aggregation.Monthly, 1262304000);
    ...
    return {
      ...
      value1: last.value1 ? records.sum('value1') : 0,
      value2: last.value2 ? records.sum('value2') : 0,
    };
  }
  ...
  async getLatestSensorConsumption(id, serviceLocationId, from) {
    ...
    const records = await this.getSensorConsumption(id, serviceLocationId, Aggregation.FiveMinutes, from);
  ...
  */
  ...
  async discoverGasWaterDevices(id = null) {
    const sensors = await this.discoverMeteringDevices('sensors');
    const devices = sensors.filter((data) => data.name.startsWith('30'));

    if (!id) return devices.all();

    return devices.where('id', id).first();
  }
