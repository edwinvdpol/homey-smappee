...
class Client extends OAuth2Client {
  ...
  */
  ...
  async getSmartDevice(serviceLocationId, deviceId) {
    const path = `/servicelocation/${serviceLocationId}/smartdevices/${deviceId}`;
  ...
  */
  ...
  async discoverConnectDevices() {
    ...
    return Promise.all(devices.map(async (device) => {
      const consumption = await this.getLatestServiceLocationConsumption(device.serviceLocationId);

      device.solar = filled(consumption.solar || null);
